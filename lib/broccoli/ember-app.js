...
EmberApp.prototype._addonTreePrep = function _addonTreePrep() {
  if (this._cachedAddonTreePrep) {
    return this._cachedAddonTreePrep;
  }
  ...
  return this._cachedAddonTreePrep = { addonTrees, reexportsAndTranspiledAddonTree };
};
...
EmberApp.prototype._addonTree = function _addonTree() {
  ...
  var trees = this._addonTreePrep();

  return this._cachedAddonTree = [
    this.concatFiles(trees.addonTrees, {
      inputFiles: ['**/*.css'],
    ...
    this.concatFiles(trees.reexportsAndTranspiledAddonTree, {
      inputFiles: ['**/*.js'],
...
EmberApp.prototype._addonModuleTree = function _addonTree() {
  if (this._cachedAddonModuleTree) {
    return this._cachedAddonModuleTree;
  }

  var trees = this._addonTreePrep();

  return this._cachedAddonModuleTree = [new Funnel(trees.reexportsAndTranspiledAddonTree, {
    inputFiles: ['**/*.js'],
    destDir: 'modules/vendor/',
    allowNone: true
  })];
};
...
EmberApp.prototype.javascript = function() {
  ...
  var appModules = new Funnel(appJs, {
    include: [this.name + '/**/*.js'],
    destDir: 'modules/',
    annotation: 'Funnel: App modules'
  });
  ...
  var appFiles = new Funnel(appJs, {
    include: appFilesInput,
    destDir: 'files/app/'
  });
  ...
  var vendorInput = ['vendor/ember-cli/vendor-prefix.js']
  ...
  var vendor = this.concatFiles(applicationJs, {
    inputFiles: vendorInput,
    outputFile: this.options.outputPaths.vendor.js,
    separator: EOL + ';',
    annotation: 'Concat: Vendor'
  });

  var vendorFilesInput = ['vendor/ember-cli/vendor-prefix.js']
    .concat(legacyFilesToAppend)
    .concat('vendor/ember-cli/vendor-suffix.js');

  var vendorFiles = new Funnel(applicationJs, {
    include: vendorFilesInput,
    destDir: 'files/vendor/'
  });

  var addonModules = mergeTrees(this._addonModuleTree());
...
EmberApp.prototype.toTree = function(additionalTrees) {
  ...
  let postprocess = this.addonPostprocessTree('all', tree);
  let manifest = ConcatFilenames(postprocess, {
    inputFiles: ['**/*.js'],
    outputFile: '/manifest.js',
    transform: function(filename) {
      return filename;
    }
  });

  return mergeTrees([manifest, postprocess]);
};
